...
export function nf1_changeMintPrice(binaryArgs: StaticArray<u8>): void {
  assert(_onlyOwner(), 'The caller is not the owner of the contract');
  const args = new Args(binaryArgs);
  const newPrice = args.nextU64().expect('');

  Storage.set(stringToBytes(mintPriceKey), u64ToBytes(newPrice));
  generateEvent('mint price is changed to');
}
...
export function nft1_mint(_args: StaticArray<u8>): void {
  ...
  const price = bytesToU64(Storage.get(stringToBytes(mintPriceKey)));
  assert(
    Context.transferredCoins() >= price,
    'Not enough sent coins to mint this NFT',
  );
  const owner = Storage.get(ownerKey);
