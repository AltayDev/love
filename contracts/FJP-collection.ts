...
export const PUBLIC_SUPPLY_KEY = stringToBytes('PUBLIC_SUPPLY');
export const ADMIN_SUPPLY_KEY = stringToBytes('ADMIN_SUPPLY');
export const MINT_PRICE_KEY = stringToBytes('MINT_PRICE');
export const COUNTER_KEY = stringToBytes('COUNTER');
export const PUBLIC_COUNTER_KEY = stringToBytes('PUBLIC_COUNTER');
export const ADMIN_COUNTER_KEY = stringToBytes('ADMIN_COUNTER');
...
export function constructor(binaryArgs: StaticArray<u8>): void {
  ...
  const maxSupply = u256.fromU32(400); // total supply is fixed at 400
  ...
  Storage.set(PUBLIC_SUPPLY_KEY, u256ToBytes(u256.fromU32(200))); // Public mint limit
  Storage.set(ADMIN_SUPPLY_KEY, u256ToBytes(u256.fromU32(200))); // Admin mint limit
  Storage.set(MINT_PRICE_KEY, u64ToBytes(mintPrice));
  Storage.set(COUNTER_KEY, u256ToBytes(u256.Zero));
  Storage.set(PUBLIC_COUNTER_KEY, u256ToBytes(u256.Zero));
  Storage.set(ADMIN_COUNTER_KEY, u256ToBytes(u256.Zero));
...
export function publicSupplyCount(
  _: StaticArray<u8> = new StaticArray<u8>(0),
): StaticArray<u8> {
  return Storage.get(PUBLIC_COUNTER_KEY);
}

export function adminSupplyCount(
  _: StaticArray<u8> = new StaticArray<u8>(0),
): StaticArray<u8> {
  return Storage.get(ADMIN_COUNTER_KEY);
}
...
export function mint(binaryArgs: StaticArray<u8>): void {
  // check public supply
  const maxPublicSupply = bytesToU256(Storage.get(PUBLIC_SUPPLY_KEY));
  assert(
    maxPublicSupply > bytesToU256(publicSupplyCount()),
    'Max public mint supply reached',
  );
  ...
  const publicIncrement = bytesToU256(publicSupplyCount()) + u256.One;

  Storage.set(COUNTER_KEY, u256ToBytes(increment));
  Storage.set(PUBLIC_COUNTER_KEY, u256ToBytes(publicIncrement));
...
export function minta(binaryArgs: StaticArray<u8>): void {
  onlyOwner();
  // check admin supply
  const maxAdminSupply = bytesToU256(Storage.get(ADMIN_SUPPLY_KEY));
  assert(
    maxAdminSupply > bytesToU256(adminSupplyCount()),
    'Max admin mint supply reached',
  );
  ...
  const to = args.nextString().expect('please enter the target address.');
  const increment = bytesToU256(currentSupply()) + u256.One;
  const adminIncrement = bytesToU256(adminSupplyCount()) + u256.One;

  Storage.set(COUNTER_KEY, u256ToBytes(increment));
  Storage.set(ADMIN_COUNTER_KEY, u256ToBytes(adminIncrement));
  _update(to, increment, '');
}
